...
//PZ DISABLE #   include <sys/limits.h>
#define NAME_MAX 255
...
int32_t ftnclos_(const int32_t * const iun);
...
int c_fnom(
    ...
    if ((FGFDT[entry].attr.old || FGFDT[entry].attr.read_only) && ! FGFDT[entry].attr.remote) {
        if (!existe_(FGFDT[entry].file_name, (F2Cl) strlen(FGFDT[entry].file_name))) {
...
static int qqcclos(
    ...
    if (FGFDT[indf].attr.remote) {
        ...
        *checksum = *s_ID ^ *addr ^ *nw ^ *RW_mode;
        // PZ DISBALE: check_swap_records(demande, 5, sizeof(int));
        // PZ DISBALE: int nc = write_stream(FGFDT[indf].fd, (char *)demande, 5 * sizeof(int));
        // PZ DISBALE: if (nc == 0) {
        // PZ DISBALE:     Lib_Log(APP_LIBRMN,APP_INFO,"%s: socket wrote to server OK\n",__func__);
        // PZ DISBALE:     fflush(stdout);
        // PZ DISBALE: } else {
        // PZ DISBALE:     Lib_Log(APP_LIBRMN,APP_ERROR,"%s: socket wrote only %d bytes to server\n",__func__,nc);
        // PZ DISBALE: }
    } else {
        if (WA_PAGE_SIZE != 0) {
            wa_pages_flush(ind);
...
void wawrit_(
    ...
    wawrit2_(iun, buf, offset, nwords);
}
...
void waread_(
    ...
    waread2_(iun, buf, offset, nwords);
}
...
static void qqcwawr(
    ...
    if (FGFDT[indf].attr.remote) {
        ...
        *checksum = *s_ID ^ *addr ^ *nw ^ *RW_mode;
        // PZ DISABLE: check_swap_records(demande, 5, sizeof(int));
        // PZ DISABLE: int nc = write_stream(FGFDT[indf].fd, (const char *)demande, 5 * sizeof(int));
        // PZ DISABLE: if (nc != 0) {
        // PZ DISABLE:     Lib_Log(APP_LIBRMN,APP_ERROR,"%s: socket wrote only %i bytes to server\n",__func__,nc);
        // PZ DISABLE: }
        // PZ DISABLE: int nelm = write_stream(FGFDT[indf].fd, (const char *)buf, nwords * sizeof(int));
#if defined (DEBUG)
        // PZ DISABLE: if (nelm == 0) Lib_Log(APP_LIBRMN,APP_DEBUG,"%s: socket wrote %d bytes\n",__func__,nwords*sizeof(int));
#endif
...
static void qqcward(
    ...
    if (FGFDT[indf].attr.remote) {
        ...
        *checksum = *s_ID ^ *addr ^ *nw ^ *RW_mode;
        ...
        // PZ DISABLE: int nc = write_stream(FGFDT[indf].fd, (const char *)demande, 5 * sizeof(int));
        // PZ DISABLE: if (nc != 0) {
        // PZ DISABLE:     Lib_Log(APP_LIBRMN,APP_ERROR,"%s: socket wrote only %d bytes to server\n",__func__,nc);
        // PZ DISABLE: }
        // PZ DISABLE:  int nelm = read_stream(FGFDT[indf].fd, (char *)buf, lnmots * sizeof(int));
#       if defined (DEBUG)
        // PZ DISABLE:     Lib_Log(APP_LIBRMN,APP_DEBUG,"%s: read %d bytes\n",__func__,nelm);
#       endif
    } else {
        if (ladr != 0) {
            ladr += wafile[ind].offset;
